...
exports.getAllProducts = async (req, res) => {
  ...
  const products = await Product.find(query).populate("supplierId", "name phone isVerified address");
    res.json(products);
  } catch (err) {
...
exports.getPublicProducts = async (req, res) => {
  try {
  const products = await Product.find({ isActive: true }).populate("supplierId", "name phone isVerified address");
    res.json(products);
  } catch (err) {
...
exports.getProductById = async (req, res) => {
  try {
  const product = await Product.findById(req.params.id).populate("supplierId", "name phone isVerified address");
...
exports.getProductsBySupplierId = async (req, res) => {
  ...
  const products = await Product.find({ supplierId, isActive: true }).populate("supplierId", "name phone isVerified address");
...
exports.getMyProducts = async (req, res) => {
  try {
    ...
    const products = await Product.find({
      supplierId: req.user.id,
      isActive: true,
    }).populate("supplierId", "name phone isVerified address");
